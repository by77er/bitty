...
one-off or as a script process when the job will recur — and the two most common of those, \
running a shell command and reading a range of a file, are already written for you as `sh()` and \
`read()` inside run_script. run_script runs in your persistent \
